...
void loop() {
  ...
  if (mount_itf != NULL) {
    ...
    FATFS  fs;
    int res =  usbfs.mount(&mount_ff, "0:", 0);
    printf("MSC mount [%s] r = %d\n", mo, res);
    mbed::fs_file_t fsrc;
    // delay(5000);
    res = usbfs.file_open(&fsrc, "0://prova.txt", FA_WRITE | FA_CREATE_ALWAYS );
    printf("open res = %d\n", res);
    //delay(100);
    ...
    // UINT br, bw;
    char buffer[10];
    res = usbfs.file_open(&fsrc, "0://prova.txt",  FA_READ | FA_OPEN_EXISTING );
    printf("open res = %d\n", res);
    res = usbfs.file_read(fsrc, buffer, sizeof(buffer));
    printf("read res = %d %s\n", res, buffer);
    res = usbfs.file_close(fsrc);
    //delay(100);
    printf("close res= %d\n", res);
